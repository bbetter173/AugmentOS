...
| Serialize `connect()` with an in-flight promise                        | Accept occasional concurrent-call races                                  | Three paths can call `connect()`: the backend_url subscription, `NO_ACTIVE_SESSION` via `handleNoActiveSession`, and the reconnect interval's `actuallyReconnect`. If two fire while the first is awaiting `detachAndCloseSocket()`'s 500ms close, the second sees `this.webSocket === null`, proceeds, and assigns its own WebSocket; the first then resumes and overwrites `this.webSocket`, orphaning the second socket server-side. This is exactly the overlap this PR is supposed to eliminate. A `connectInFlight: Promise<void> \| null` field makes callers serialize. (Caught by CodeRabbit on PR #2592.) |
...
**Risk: race between a `backend_url` write and a pending WS reconnect.** Handled by the `connectInFlight` in-flight guard (see Decision Log). Concurrent callers serialize: the second caller awaits the first's `connect()` promise before starting its own. The first still completes against the pre-change URL, then the second picks up the new URL via `getCurrentWsUrl()` inside its own `performConnect`. One extra cycle maximum, no overlap, no orphaned sockets.
...
One cached field removed (`this.url`), one Zustand subscription added, one in-flight promise field added to serialize concurrent reconnects, two helpers made async, one React component useEffect tightened up. About 340 lines added and 160 removed across 5 files. No new dependencies, no new state machines, no rate limiters, no debouncers. Addresses the actual root cause, closes the dev-settings UX gap that always-derive alone left open, and defends against the concurrent-reconnect race the overlap-await window opened up.
