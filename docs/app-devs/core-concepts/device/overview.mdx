...
| Field | Type | Description |
|---|---|---|
| `event.gesture` | `string` | Gesture name (e.g. `"single_tap"`, `"double_tap"`, `"swipe_forward"`, `"swipe_backward"`) |
...
const stop = session.device.onBatteryUpdate((event) => {
  ...
  if (event.timeRemaining !== undefined) {
    console.log(`Time remaining: ${event.timeRemaining} min`);
  }
});
```
...
| `event.timeRemaining` | `number \| undefined` | Estimated minutes remaining (if available) |
...
Subscribe to capability changes. Fires on initial connection and whenever capabilities are updated.
...
app.onSession((session) => {
  ...
  session.device.onTouchEvent("swipe_forward", () => {
