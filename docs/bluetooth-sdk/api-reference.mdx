...
Use `scan()` for user-facing device pickers. The progressive result callback is for UI: render the nearby-device list every time it changes during scanning. The returned final result is for control flow: after the timeout/completion, choose a device from the last list and connect.
...
| `rssi` | Optional signal strength from scan results. It may be absent when the platform scan result does not include it yet. |
