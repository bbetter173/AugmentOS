...
import com.mentra.asg_client.io.streaming.config.WebRtcStreamConfig;
...
/**
 * WebRTC streaming service that streams camera + audio to a WHIP endpoint.
 *
 * WHIP (WebRTC-HTTP Ingest Protocol) flow:
 ...
 *   WebRtcStreamingService.startStreaming(context, whipUrl, streamId, enableLed, enableSound, config)
 *   WebRtcStreamingService.stopStreaming(context)
 *   WebRtcStreamingService.isStreaming()
 */
@SuppressLint("MissingPermission")
public class WebRtcStreamingService extends Service {

    private static final String TAG = "WebRtcStreamingService";
    private static final String CHANNEL_ID = "WebRtcStreamingChannel";
    private static final int NOTIFICATION_ID = 8890;

    // Static instance so static helper methods can reach the running service
    private static WebRtcStreamingService sInstance;
    private static StreamingStatusCallback sStatusCallback;
    private static WebRtcStreamConfig sPendingStreamConfig = null;
    ...
    private WebRtcStreamConfig mStreamConfig = new WebRtcStreamConfig();
    ...
    // HTTP client for WHIP signaling (reuses the OkHttp instance already in the app)
    ...
    public void onCreate() {
        ...
        Log.d(TAG, "WebRtcStreamingService created");
    }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (intent != null) {
            ...
            if (whipUrl != null && !whipUrl.isEmpty()) {
                ...
                // Short delay to let the service fully start before streaming
    ...
    public void onDestroy() {
        ...
        stopStreaming();
        Log.d(TAG, "WebRtcStreamingService destroyed");
        super.onDestroy();
    }
    ...
    private void startStreaming() {
        ...
        Log.d(TAG, "Starting WebRTC streaming to " + mWhipUrl);
    ...
    private void stopStreaming() {
        ...
        Log.d(TAG, "Stopping WebRTC streaming");

        // Send HTTP DELETE to the WHIP resource URL for clean teardown
        ...
        Log.d(TAG, "WebRTC streaming stopped");
    }
    ...
    private void setupCamera() {
        ...
        mVideoCapturer = enumerator.createCapturer(cameraId, null /* CameraEventsHandler */);

        mSurfaceTextureHelper = SurfaceTextureHelper.create(
                "WebRtcCaptureThread", mEglBase.getEglBaseContext());

        mVideoSource = mPeerConnectionFactory.createVideoSource(false /* isScreencast */);
    ...
    private void setupAudio() {
        ...
        audioConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
                ...
                String.valueOf(mStreamConfig.isEchoCancellation())));
        audioConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
                "googNoiseSuppression",
                String.valueOf(mStreamConfig.isNoiseSuppression())));
        audioConstraints.mandatory.add(new MediaConstraints.KeyValuePair(
    ...
    private void createPeerConnectionAndOffer() {
        ...
        // Unified Plan is required for modern WebRTC
        rtcConfig.sdpSemantics = PeerConnection.SdpSemantics.UNIFIED_PLAN;
        // Gather once — WHIP sends the full offer after COMPLETE fires
        ...
        // Add tracks (UNIFIED_PLAN: addTrack, not addStream)
        mPeerConnection.addTrack(mVideoTrack);
        mPeerConnection.addTrack(mAudioTrack);

        // WHIP requires SEND_ONLY — addTrack() defaults to SEND_RECV which causes
        // Cloudflare (and most WHIP servers) to accept the connection but drop media.
            ...
            public void onCreateSuccess(SessionDescription offer) {
                ...
                mPeerConnection.setLocalDescription(new SdpObserver() {
                    ...
                    public void onSetSuccess() {
                        ...
                        // ICE gathering completion is handled in WhipPeerConnectionObserver
                    }
    ...
    /**
     * POST the SDP offer to the WHIP URL.
     * Called once ICE gathering is complete so the offer contains all candidates.
     */
    private void postOfferToWhip(SessionDescription offer) {
        ...
        mHttpClient.newCall(request).enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                // Store the resource URL for later HTTP DELETE
    ...
    /** Send HTTP DELETE to cleanly tear down the WHIP session on the server. */
    ...
    private class WhipPeerConnectionObserver implements PeerConnection.Observer {
        ...
        public void onIceGatheringChange(PeerConnection.IceGatheringState newState) {
            ...
            if (newState == PeerConnection.IceGatheringState.COMPLETE) {
                // All ICE candidates are now embedded in the local SDP — safe to send
        ...
        public void onIceCandidate(IceCandidate candidate) {
            // With non-trickle WHIP we wait for COMPLETE, so no action needed here
        }
    ...
    private void releaseWebRtc() {
        ...
        // Dispose factory and EGL context last
    ...
    private void notifyStarting(String url) {
        if (sStatusCallback != null) {
            mMainHandler.post(() -> sStatusCallback.onStreamStarting(url));
        }
    }

    private void notifyStarted(String url) {
        if (sStatusCallback != null) {
            mMainHandler.post(() -> sStatusCallback.onStreamStarted(url));
        }
    }

    private void notifyStopped() {
        if (sStatusCallback != null) {
            mMainHandler.post(() -> sStatusCallback.onStreamStopped());
        }
    }

    private void notifyError(String error) {
        if (sStatusCallback != null) {
            mMainHandler.post(() -> sStatusCallback.onStreamError(error));
        }
    }
    ...
    private void createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            channel.setDescription("Shows when the app is streaming via WebRTC/WHIP");
    ...
    private Notification createNotification(String status) {
        return new NotificationCompat.Builder(this, CHANNEL_ID)
                .setContentTitle("MentraOS WebRTC Streaming")
    ...
    /** Select the first back-facing camera; falls back to the first available camera. */
    ...
    /**
     * Build an absolute URL from a WHIP base URL and a (potentially relative) Location path.
     * e.g. base="https://ingest.example.com/whip/live", location="/whip/live/abc123"
     * → "https://ingest.example.com/whip/live/abc123"
     */
    ...
    // -----------------------------------------------------------------------
    // Static public API  (mirrors RtmpStreamingService)
    // -----------------------------------------------------------------------
     ...
     * @param whipUrl   WHIP ingest URL (e.g. https://ingest.example.com/whip/live)
     * @param streamId  Optional stream ID for tracking
     * @param enableLed Whether to enable the recording LED
     * @param enableSound Whether to play start/stop sounds
     * @param config    Video/audio configuration (null = defaults)
     */
    public static void startStreaming(Context context, String whipUrl, String streamId,
            boolean enableLed, boolean enableSound, WebRtcStreamConfig config) {
        setStreamConfig(config);

        if (sInstance != null) {
            // Service already running — update URL and start directly
            ...
            Intent intent = new Intent(context, WebRtcStreamingService.class);
    ...
    /** @see #startStreaming(Context, String, String, boolean, boolean, WebRtcStreamConfig) */
    public static void startStreaming(Context context, String whipUrl, String streamId,
    ...
    /** @see #startStreaming(Context, String, String, boolean, boolean, WebRtcStreamConfig) */
    public static void startStreaming(Context context, String whipUrl, String streamId) {
        startStreaming(context, whipUrl, streamId, true, true, null);
    }

    /** @see #startStreaming(Context, String, String, boolean, boolean, WebRtcStreamConfig) */
    public static void startStreaming(Context context, String whipUrl) {
        startStreaming(context, whipUrl, null, true, true, null);
    }

    /**
     * Stop the active WebRTC stream and tear down the WHIP session.
     *
     * @param context Android context
     */
    public static void stopStreaming(Context context) {
        ...
        context.stopService(new Intent(context, WebRtcStreamingService.class));
    }
    ...
    /**
     * Register a callback to receive streaming status events.
     ...
     * May be called before or while the service is running.
     *
     * @param config Stream configuration (null = keep current / defaults)
     */
    public static void setStreamConfig(WebRtcStreamConfig config) {
