...
Status snapshots are safe to read at any time in the native SDKs. In React Native, keep UI state derived from `useMentraBluetooth()` and typed `useBluetoothEvent()` handlers. Treat command success as "command accepted"; keep UI state derived from status callbacks or hook state.
