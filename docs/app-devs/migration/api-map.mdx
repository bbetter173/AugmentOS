...
| v2 | v3 | Notes |
|----|-----|-------|
| `session.audio.play(url)` | `session.speaker.play(url)` | Same signature. |
...
| `session.camera.startStream(options)` | `session.camera.startStream(options)` | Same API surface. |
| `session.camera.stopStream()` | `session.camera.stopStream()` | Same API surface. |
