...
Only documented root imports are part of the supported app developer API. Package subpaths or symbols with a leading underscore are private implementation details and can change without notice.
...
The React Native root export intentionally exposes a narrower surface than the MentraOS internal adapter. These methods are the supported app developer entrypoints:
...
The React Native root export does not expose raw native status getters or raw status events. In app code, do not call `getGlassesStatus()`, `getBluetoothStatus()`, `onGlassesStatus()`, `onBluetoothStatus()`, or subscribe to `glasses_status` / `bluetooth_status`. Those native store details are private implementation hooks used by `@mentra/bluetooth-sdk/react` to produce shaped `mentra.glasses`, `mentra.sdk`, and `mentra.scan` state.
...
The React Native event surface is typed through `BluetoothSdkEventMap`. For example, `BluetoothSdk.addListener('photo_response', ...)` receives a `PhotoResponseEvent`, while `BluetoothSdk.addListener('mic_pcm', ...)` receives a `MicPcmEvent`. `MicPcmEvent` includes `sampleRate`, `bitsPerSample`, `channels`, `encoding`, and `vadGated`; `MicLc3Event` includes `sampleRate`, `channels`, `encoding`, `frameDurationMs`, `frameSizeBytes`, `bitrate`, `packetizedFromGlasses`, and `vadGated`.
...
Android and iOS receive the same event families through their SDK callback and delegate surfaces. Microphone audio callbacks use `MicPcmEvent` and `MicLc3Event` objects with the same metadata as React Native.
