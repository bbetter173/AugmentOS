...
const app = new MiniAppServer({ packageName: "com.example.led", apiKey: process.env.API_KEY!, port: 3000 });

app.onSession((session: MentraSession) => {
  // Always check LED capabilities first
  if (!session.capabilities?.hasLight) {
    session.display.showTextWall("No LED support on this device");
    return;
  }
...
if (!session.capabilities?.hasLight) {
  ...
  // Or audio feedback
  await session.speaker.speak("Notification received");
}
```
