...
The dashboard is a persistent view that appears when the user looks up. It's separate from your app's main display. Use it for status information, time, weather, notifications, or anything the user wants to glance at quickly.

## Basic Usage

```typescript
session.dashboard.showText("New message received");
```

With title and content:

```typescript
session.dashboard.showText("App Status\nConnected and running");
```
...
```typescript
session.dashboard.showText("Status: Active");
```
...
```typescript
session.dashboard.showText("Detailed status information here");
```
...
session.transcription.on(async (data) => {
  if (data.isFinal) {
    ...
    session.display.showTextWall(response);
