...
type TcpSocketModule = typeof TcpSocket
...
class MiniSockets {
  ...
  public sendAudio(audio: ArrayBuffer) {
    ...
    for (const [id, client] of this.clients) {
      ...
      try {
        ...
        console.error(`MINISOCKET: Error sending audio to client ${id}:`, errorMessage(err))
  ...
  public sendMessage(message: object) {
    ...
    for (const [id, client] of this.clients) {
      ...
      try {
        ...
        console.error(`MINISOCKET: Error sending message to client ${id}:`, errorMessage(err))
        this.removeClient(id)
      }
    }
  }
  ...
  public sendToClient(clientId: number, message: object): boolean {
    ...
    try {
      ...
      console.error(`MINISOCKET: Error sending to client ${clientId}:`, errorMessage(err))
      this.removeClient(clientId)
      return false
    }
  }
  ...
  public sendRawToClient(clientId: number, raw: string): boolean {
    ...
    try {
      ...
      console.error(`MINISOCKET: Error sending raw to client ${clientId}:`, errorMessage(err))
      this.removeClient(clientId)
      return false
    }
  }
  ...
  private getTcpSocket(): TcpSocketModule {
    if (!this.tcpSocket) {
      const tcpSocketModule = require("react-native-tcp-socket") as {default: TcpSocketModule}
      this.tcpSocket = tcpSocketModule.default
    }
