...
Create `cloud/packages/miniapp/` (or `mobile/packages/miniapp/` if the monorepo layout prefers mobile-adjacent packages — pick one and stick with it).

```
cloud/packages/miniapp/
├── package.json
├── tsconfig.json
├── src/
...
**Legacy bridge messages (`share`, `copy_clipboard`, `open_url`, `download`, `core_fn`):** delete from `MiniComms.ts` and `@mentra/react`. These are not in the published `@mentra/react` 2.1.2 tarball (see the React SDK row in Key Files). If any of this functionality is needed later, add as SDK methods (e.g., `session.system.share()`).
...
**Mobile protocol enum import:** add `"@mentra/miniapp": "file:../cloud/packages/miniapp"` to `mobile/package.json` (same pattern as `@mentra/display-utils`). Mobile then imports `import { MiniappRequestType } from "@mentra/miniapp/protocol"`. Requires `@mentra/miniapp` to be built (`dist/` present) before mobile's TypeScript pass.
...
- `cloud/packages/miniapp/src/session.ts` — add `visibility`, `onVisibilityChange`, `PONG` auto-reply handler.
- `cloud/packages/miniapp/src/protocol.ts` — add `MiniappResponseType.VISIBILITY_CHANGE`, `MiniappRequestType.PING`, `MiniappResponseType.PONG` enum values.
...
A new CLI package (`cloud/packages/create-mentra-miniapp/`) that scaffolds a working miniapp project.
...
- The scaffolder's own `package.json` in `cloud/packages/create-mentra-miniapp/` uses `workspace:*` for its own deps (monorepo-internal).
- Publication order: `@mentra/miniapp` and `@mentra/miniapp-cli` must be on npm before `create-mentra-miniapp` is published. For internal pre-publication testing, swap the template deps to `"file:../../cloud/packages/miniapp"` while running from the monorepo.

**Scaffolder script:** `cloud/packages/create-mentra-miniapp/bin/index.ts` — Bun script that copies `template/` to target path, replaces `{{packageName}}` placeholders in `miniapp.json`, prints next-step instructions.
...
New package at `cloud/packages/miniapp-cli/`. Commands: `dev` and `pack`.
...
4. Validates declared permissions against a runtime allowlist. `AppPermissionType` in `cloud/packages/types/src/applet.ts:15` is a TypeScript `type` alias (not an enum), so the CLI defines its own runtime constant:
   ...
   ```ts
   // cloud/packages/miniapp-cli/src/manifest.ts
...
- `cloud/packages/miniapp/src/modules/camera.ts` — `CameraModule.takePhoto()` sends `miniapp_request` with `type: "camera.take_photo"`, waits for `miniapp_response`
...
**`cloud/packages/miniapp/src/modules/stream.ts`** — `StreamModule.startUnmanaged(rtmpUrl, options)` returns `AsyncIterable<StreamStatus>` (or a Promise+event-emitter, matching current `@mentra/sdk` shape). Waits for the first `ACTIVE` status before resolving.
...
- `cloud/packages/miniapp/src/modules/stream.ts` — `StreamModule.startUnmanaged`, `startManaged`, `stop`
...
1. **Phase 1** — Create `@mentra/miniapp` package (`protocol.ts`, transports, `MiniappSession`, modules, React hooks). Add `build:miniapp` to root `cloud/package.json` (and later `build:create-mentra-miniapp` + `build:miniapp-cli` for Phase 3 — those two are dev-machine tools and aren't required for cloud builds). No phone or cloud changes in this phase.
...
1. **Envelope parsing** (`cloud/packages/miniapp/src/envelope.test.ts`) — encode/decode round-trip, malformed input rejection, `requestId` handling, binary base64.
...
5. **Protocol enum wire values** (`cloud/packages/miniapp/src/protocol.test.ts`) — assert enum values match expected strings. Catches rename drift.
...
| File                                                   | Change                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `cloud/packages/miniapp/package.json`                  | NEW — miniapp SDK package manifest (see Phase 1.2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cloud/packages/miniapp/tsconfig.json`                 | NEW — browser-targeted TS config (no Node lib, no Node types)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `cloud/packages/miniapp/src/index.ts`                  | NEW — public exports (MiniappSession + types)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `cloud/packages/miniapp/src/session.ts`                | NEW — `MiniappSession` class                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `cloud/packages/miniapp/src/protocol.ts`               | NEW — wire protocol enums (`MiniappRequestType`, `MiniappResponseType`, `MiniappStreamType`). Fresh miniapp-naming, no legacy `tpa_*`. See Phase 1.3.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| `cloud/packages/miniapp/src/envelope.ts`               | NEW — bridge envelope format `{kind, payload, requestId}`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `cloud/packages/miniapp/src/transport/types.ts`        | NEW — Transport interface                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `cloud/packages/miniapp/src/transport/postmessage.ts`  | NEW — WebView bridge transport                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `cloud/packages/miniapp/src/transport/local-socket.ts` | NEW — browser WebSocket transport                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `cloud/packages/miniapp/src/transport/auto.ts`         | NEW — auto-detection                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `cloud/packages/miniapp/src/modules/layouts.ts`        | NEW — LayoutManager                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `cloud/packages/miniapp/src/modules/events.ts`         | NEW — EventManager (eventemitter3)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cloud/packages/miniapp/src/modules/audio.ts`          | NEW — `play()`, `speak()` (sends SPEAK request — phone constructs TTS URL)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `cloud/packages/miniapp/src/modules/camera.ts`         | NEW — `takePhoto()` noop+warn in v1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `cloud/packages/miniapp/src/modules/dashboard.ts`      | NEW — `DashboardAPI.setContent()` noop+warn in v1 (deferred)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `cloud/packages/miniapp/src/modules/led.ts`            | NEW — LedModule                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `cloud/packages/miniapp/src/modules/storage.ts`        | NEW — SimpleStorage (bridge to phone AsyncStorage)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `cloud/packages/miniapp/src/react/index.ts`            | NEW — React hooks exports (v1: only `useSession`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `cloud/packages/miniapp/src/react/useSession.ts`       | NEW — zero-config session hook                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `cloud/package.json`                                   | Add build scripts and chain `build:miniapp` into the root `build` and `postinstall`. `build:create-mentra-miniapp` and `build:miniapp-cli` are developer-machine only and not chained into the cloud build.<br>`"build:miniapp": "cd packages/miniapp && bun run build"`<br>`"build:create-mentra-miniapp": "cd packages/create-mentra-miniapp && bun run build"`<br>`"build:miniapp-cli": "cd packages/miniapp-cli && bun run build"`<br>Update root `build`: `"build": "bun run build:types && bun run --parallel build:display-utils build:utils && bun run build:sdk && bun run build:miniapp"`. Update `postinstall` to include `build:miniapp` so cloud deploys pick up the package. |

### Unit tests (Phase 1 + Phase 2)

| File                                                                           | Change                                                              |
| ------------------------------------------------------------------------------ | ------------------------------------------------------------------- |
| `cloud/packages/miniapp/src/envelope.test.ts`                                  | NEW — envelope parse/serialize roundtrip, malformed input rejection |
| `cloud/packages/miniapp/src/protocol.test.ts`                                  | NEW — assert wire values match expected strings                     |
...
| File                                                          | Change                                                                                                                                   |
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| `cloud/packages/create-mentra-miniapp/package.json`           | NEW — scaffolder CLI package manifest, `bin` entry for `create-mentra-miniapp`                                                           |
| `cloud/packages/create-mentra-miniapp/bin/index.ts`           | NEW — scaffolder entry point. Copies `template/` to target path, replaces `{{packageName}}` placeholders, prints next-step instructions. |
| `cloud/packages/create-mentra-miniapp/template/package.json`  | NEW — starter project package.json with `dev`/`build`/`pack` scripts, `@mentra/miniapp` + `react` + `react-dom` deps                     |
| `cloud/packages/create-mentra-miniapp/template/server.ts`     | NEW — `Bun.serve({ development: { hmr: true, console: true } })` entry with HTML import                                                  |
| `cloud/packages/create-mentra-miniapp/template/index.html`    | NEW — HTML entry with `<script type="module" src="./src/main.tsx">`                                                                      |
| `cloud/packages/create-mentra-miniapp/template/src/main.tsx`  | NEW — React root mount                                                                                                                   |
| `cloud/packages/create-mentra-miniapp/template/src/App.tsx`   | NEW — starter component using `useSession` from `@mentra/miniapp/react`                                                                  |
| `cloud/packages/create-mentra-miniapp/template/miniapp.json`  | NEW — manifest template with `{{packageName}}` placeholder                                                                               |
| `cloud/packages/create-mentra-miniapp/template/icon.png`      | NEW — default icon                                                                                                                       |
| `cloud/packages/create-mentra-miniapp/template/tsconfig.json` | NEW — react-jsx target                                                                                                                   |

### New package: `@mentra/miniapp-cli` (dev helper + packager)

| File                                         | Change                                                                                                                                                                                                                                  |
| -------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `cloud/packages/miniapp-cli/package.json`    | NEW — CLI package manifest, `bin` entries for `mentra-miniapp` command                                                                                                                                                                  |
| `cloud/packages/miniapp-cli/src/index.ts`    | NEW — CLI entry point with `dev` and `pack` subcommands                                                                                                                                                                                 |
| `cloud/packages/miniapp-cli/src/dev.ts`      | NEW — `mentra-miniapp dev` implementation: detect LAN IP via `os.networkInterfaces()`, construct `mentra-miniapp://dev?url=...` URL, print QR code to terminal via `qrcode-terminal`, print URL as fallback, monitor for LAN IP changes |
| `cloud/packages/miniapp-cli/src/pack.ts`     | NEW — `mentra-miniapp pack` implementation: verify `dist/` exists, copy `miniapp.json` + `icon.png` into `dist/`, validate manifest schema + permissions, create `<packageName>-<version>.zip`                                          |
| `cloud/packages/miniapp-cli/src/qr.ts`       | NEW — QR code generation helper using `qrcode-terminal`                                                                                                                                                                                 |
| `cloud/packages/miniapp-cli/src/manifest.ts` | NEW — `miniapp.json` schema validation + `AppPermissionType` validation                                                                                                                                                                 |
