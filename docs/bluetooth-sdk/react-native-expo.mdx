...
<Warning>
The SDK does not currently use `CBCentralManagerOptionRestoreIdentifierKey` on iOS. Terminated-app Core Bluetooth state restoration is not supported yet. If iOS terminates the app, relaunch the app and reconnect from your normal startup flow.
</Warning>
...
The root `BluetoothSdk` export intentionally does not expose raw native status getters or raw `glasses_status` / `bluetooth_status` events. Use `useMentraBluetooth()` for connection, battery, Wi-Fi, hotspot, scan, and SDK runtime state.
