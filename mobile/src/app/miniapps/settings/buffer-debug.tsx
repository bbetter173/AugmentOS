...
export default function BufferDebugPage() {
  ...
  const handleSave = async (seconds: number) => {
    ...
    await CoreModule.saveBufferVideo(requestId, seconds)
    Toast.show({
      type: "success",
  ...
  const handleVideoStartStop = async () => {
    if (isVideoRecording) {
      if (videoRequestId) {
        await CoreModule.stopVideoRecording(videoRequestId)
      ...
      await CoreModule.startVideoRecording(requestId, true, false, true)
