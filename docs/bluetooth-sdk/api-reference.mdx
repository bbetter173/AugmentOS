...
Use `scan()` for user-facing device pickers. The progressive result callback is for UI: render the current nearby-device list every time it changes while Bluetooth is still scanning. The returned final result is for control flow: after the timeout/completion, choose a device from the last list and connect. React Native `connectFirst(model, options)` is still available for kiosk-style flows where the app can safely connect to the first matching device.
...
Public `GlassesStatus` fields:

| Field | Meaning |
| --- | --- |
| `connection` | Discriminated connection state. `fullyBooted` exists only when connected. |
| `micEnabled`, `btcConnected` | Microphone and Bluetooth-classic/audio state reported by the connected model. |
...
| `androidVersion`, `appVersion`, `buildNumber`, `fwVersion`, `besFwVersion`, `mtkFwVersion` | Version fields. See Version Fields below for how to display them. |
| `btMacAddress`, `leftMacAddress`, `rightMacAddress` | Model-specific Bluetooth addresses when available. |
...
| `gallery_mode` | Current gallery-button behavior setting known to the SDK. |
...
| Area | Methods |
| --- | --- |
| Status and subscriptions | `getGlassesStatus`, `getBluetoothStatus`, `onGlassesStatus`, `onBluetoothStatus`, `addListener` |
| Default device | `getDefaultDevice`, `setDefaultDevice`, `clearDefaultDevice` |
| Connection | `scan`, `startScan`, `stopScan`, `connectFirst`, `connect`, `connectDefault`, `cancelConnectionAttempt`, `disconnect`, `forget` |
| Display | `displayText`, `clearDisplay`, `showDashboard`, `setBrightness`, `setAutoBrightness`, `setDashboardPosition`, `setHeadUpAngle`, `setScreenDisabled` |
| Wi-Fi and hotspot | `requestWifiScan`, `sendWifiCredentials`, `forgetWifiNetwork`, `setHotspotState` |
| Camera and gallery | `photoRequest`, `queryGalleryStatus`, `setGalleryMode`, `setButtonPhotoSettings`, `setButtonVideoRecordingSettings`, `setButtonCameraLed`, `setButtonMaxRecordingTime`, `setCameraFov`, `startVideoRecording`, `stopVideoRecording` |
...
React Native helper exports include `DeviceModels`, `createDisconnectedGlassesStatus`, `isConnectedGlassesConnectionStatus`, `isReadyGlassesConnectionStatus`, `isBusyGlassesConnectionStatus`, `isConnectedWifiStatus`, and `isEnabledHotspotStatus`.
...
- `connectFirst(model, options)` scans until the first matching device is found, connects to it, and times out after 15 seconds unless `options.timeoutMs` is set.
- `connect`, `connectDefault`, and `connectFirst` default `saveAsDefault` and `cancelExistingConnectionAttempt` to `true`.
- `displayText(text, x, y, size)` defaults to `x = 0`, `y = 0`, and `size = 24`.
- `setBrightness(level)` leaves auto-brightness unchanged; pass `setBrightness(level, false)` or call `setAutoBrightness(false)` when you need manual brightness.
- `setMicState(enabled)` defaults to glasses microphone on, VAD bypass off, transcript events off, and LC3 events off.
- `photoRequest(..., authToken, ...)` omits the `Authorization` header when `authToken` is `null` or empty.
...
| Connection | `scan`, `connectFirst`, `startScan`, `stopScan`, `connect`, `connectDefault`, `disconnect`, `forget` |
| Wi-Fi and hotspot | `requestWifiScan`, `sendWifiCredentials`, `forgetWifiNetwork`, `setHotspotState` |
